...
class ShapeNetRender:
    ...
    def __choose(self, category):
        ...
        selection = random.choice(models)  # random choice
    ...
    def __join(self):
        for obj in bpy.data.objects:
            print(self.staging_path)
            print(obj.name)
            if 'mesh' in obj.name or 'Mesh' in obj.name or 'SketchUp' in obj.name or 'group' in obj.name\
                    or 'instance' in obj.name or 'Group' in obj.name:
                obj.select = True
