...
class BlenderRender:
    ...
    def __init__(self, gpu=False, spotlight=False):
        if gpu:
            self.switchToGPU()
    ...
    def reset_scene(self, ignore=['Camera','sphere'], gpu=False, spotlight=False):
        if gpu:
            self.switchToGPU()
        self.delete(lambda x: x.name not in ignore)
    ...
    def light(self, energy, pos):
        ...
        # bpy.ops.object.lamp_add(type='SUM', location=(0,5,0))

    def add_light(self, energy, pos, name="New Lamp"):
